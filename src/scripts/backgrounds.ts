...
const THEMES: Record<Theme, Runner> = {
  /* Cosmic — particles orbiting a gentle gravity well, faint constellations.
   ...
   is replaced by FIELD (cursor-seeded living field below); every other cosmic
   page keeps the gravity-well orbit byte-identical. The home flag is set by
   Home.astro's module script, which runs (module = deferred) long before
   ThemeBackground's load-time import resolves. */
  cosmic(ctx) {
...
/* Field — the home page's living background: cosmic motes at rest, stirred by
   ...
   inside `cosmic` only for the home page (documentElement[data-page='home']):
   the BgTheme union is owned by the experiment registry (out of scope to
   extend), same as the PAPER selection. Reduced motion never reaches here
   (ThemeBackground returns before importing). The ~30fps cap, the resize
   handling and the hidden-tab pause all come from startBackground. */
const FIELD: Runner = (ctx) => {
  const { c } = ctx;
  const N = Math.min(90, Math.floor((ctx.w * ctx.h) / 16000));
